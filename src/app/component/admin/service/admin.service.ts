...
export class AdminService {
  ...
  addAdminToCollectionIfMissing<Type extends Pick<IAdmin, 'id'>>(
    ...
    // recuperer l'admin connecté
    getAdminByTelephone(telephone: string): Observable<HttpResponse<IAdmin>> {
        return this.http.get<IAdmin>(`${this.resourceUrl}/get-by-telephone/${telephone}`, { observe: 'response' });
    }

    // get all admins
    getAllAdmins(): Observable<EntityArrayResponseType> {
        return this.http.get<IAdmin[]>(`${this.resourceUrl}/get-all`, { observe: 'response' });
    }

  createAdmin(admin: any): Observable<EntityResponseType> {
    return this.http.post<IAdmin>(`${this.resourceUrl}/create`, admin, { observe: 'response' });
  }
